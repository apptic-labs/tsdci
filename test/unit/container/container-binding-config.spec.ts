...
const mockInjectorInstrumentConstructor =
	InjectorHandler.instrumentConstructor as jest.Mock;
const mockInjectorBlockInstantiation =
	InjectorHandler.blockInstantiation as jest.Mock;
const mockInjectorUnBlockInstantiation =
	InjectorHandler.unblockInstantiation as jest.Mock;
const mockInjectorGetConstructorFromType =
...
describe("IoCBindConfig", () => {
	...
	describe("instrumentConstructor()", () => {
		it("should instrument the type constructor", () => {
			...
			const bindConfig = new IoCBindConfig(
				MyBaseType,
				mockInstanceFactory,
				mockValueFactory,
			);
			...
			expect(mockInjectorInstrumentConstructor).toHaveBeenCalledWith(
				MyBaseType,
			);
			expect(MyBaseType.constructor).toStrictEqual(decoratedConstructor);
			expect(bindConfig.decoratedConstructor).toStrictEqual(
	...
	describe("getInstance()", () => {
		it("shoud retrieve instances, creating them based on the configuration state", () => {
			...
			expect(bindConfig.iocScope.resolve).toHaveBeenCalledWith(
				bindConfig.iocFactory,
				MyBaseType,
				buildContext,
			);
		});
	...
	describe("to()", () => {
		...
		it("should pass parameters for type constructor in the generated provider", () => {
			...
			mockInstanceFactory.mockImplementation((type) =>
				type === Date ? new Date() : null,
			);
			mockInjectorGetConstructorFromType.mockReturnValue(MyType);
			const bindConfig = new IoCBindConfig(
				MyType,
			...
			expect(mockInjectorInjectContext).toHaveBeenCalledWith(
				MyType,
				buildContext,
			);
			expect(mockInjectorUnBlockInstantiation).toHaveBeenCalled();
